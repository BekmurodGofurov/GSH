...
export function KpiStatsGrid({ kpis }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 lg:gap-5">
      {/* 1. Online Servers */}
      <Card className="p-5 border-emerald-500/20 bg-gradient-to-br from-slate-900/90 to-emerald-950/10">
        ...
        <div className="mt-4">
          <div className="flex justify-between text-[11px] font-mono text-slate-400 mb-1">
            ...
            <span className="text-emerald-400 font-semibold">{kpis.uptimePercentage}%</span>
          </div>
      ...
      {/* 2. Average Ping */}
      <Card className="p-5 border-cyan-500/20 bg-gradient-to-br from-slate-900/90 to-cyan-950/10">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-mono text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
        ...
        <div className="mt-4 flex items-center justify-between text-[11px] font-mono text-slate-400 pt-2 border-t border-slate-800/80">
          <span>Target Threshold: &lt;50ms</span>
          <span className="text-cyan-400 flex items-center gap-1">
            <TrendingDown className="w-3 h-3 text-emerald-400" /> Nominal
          </span>
        </div>
      </Card>

      {/* 3. Total Active Players */}
      <Card className="p-5 border-violet-500/20 bg-gradient-to-br from-slate-900/90 to-violet-950/10">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-mono text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
        ...
        <div className="mt-4">
          <div className="flex justify-between text-[11px] font-mono text-slate-400 mb-1">
            ...
            <span className="text-violet-400 font-semibold">{playerPercentage}%</span>
          </div>
          <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
            <div
      ...
      {/* 4. Health Index */}
      <Card className="p-5 border-slate-700/60 bg-gradient-to-br from-slate-900/90 to-slate-950">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-mono text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="mt-2 flex items-baseline gap-2">
              <span className={`text-3xl font-black font-mono tracking-tight ${kpis.healthIndex > 80 ? 'text-emerald-400' : kpis.healthIndex > 60 ? 'text-amber-400' : 'text-rose-400'}`}>
                {kpis.healthIndex}%
              </span>
            </div>
          </div>
        ...
        <div className="mt-4 flex items-center justify-between text-[11px] font-mono text-slate-400 pt-2 border-t border-slate-800/80">
          ...
          <span className="text-slate-200">UDP / A2S 128T</span>
        </div>
      </Card>
    </div>
  );
